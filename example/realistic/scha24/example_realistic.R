...
realtide <- realistic_tide(plot=FALSE,fixed_seed=12)
...
initial_sigma2_zeta=1.e-7 #6.5e-6
...
write.csv(format(df,digits=3,scientific=FALSE) ,file="realistic_scha24.csv",quote=FALSE,row.names=FALSE)


# histograms of parameters
dev.new(width=6,height=9)
par(mfrow=c(3,2),mar = c(5, 5, 2, 1))
hist(sc_out$sigma2_epsilon[1501:3000],xlab="",main=expression(epsilon))
hist(sc_out$sigma2_zeta[1501:3000],xlab="",ylab="",main=expression(zeta))
hist(sc_out$sigma2_kappa[1501:3000,1],xlab="",main=expression(kappa[1]))
hist(sc_out$sigma2_kappa[1501:3000,2],xlab="",ylab="",main=expression(kappa[2]))
hist(sc_out$sigma2_kappa[1501:3000,3],xlab="",main=expression(kappa[3]))
hist(sc_out$sigma2_kappa[1501:3000,4],xlab="",ylab="",main=expression(kappa[4]))
...
ip1b <- rm_reference_phase(-1*sc_out$phase_thinned$M4,
                           chirp[,"D4_phasearg"])
ip1b <- sweep(ip1b,2,chirp[,"D4_phase"],"+")
ip1b <- aggregate_iter(ip1b,is_angle=TRUE)
...
write.csv(format(archive_df,digits=5),file="chirp24.csv",quote=FALSE,row.names=FALSE)
