...
wav = pywt.ContinuousWavelet('cmor2.5-0.96')
...
for n, scale in enumerate(scales):
    ...
    axes[n, 1].set_xticks([-np.pi/2, 0, np.pi/2])
    axes[n, 1].set_xticklabels([r'$-\pi/2$', '0', r'$\pi/2$'])
    axes[n, 1].grid(True, axis='x')
    axes[n, 1].text(np.pi/4, 0.5, 'scale = {}'.format(scale))
